...
const arc=JSON.parse(await fs.readFile('public/models/arc-pack/manifest.json','utf8'));
paths.push(...arc.map(a=>a.file));
...
for(const name of [...new Set(paths)]) {
  ...
  const skin=[...skins,...arc].find(skin=>skin.file===name);if(skin)assert.equal(crypto.createHash('sha256').update(bytes).digest('hex'),skin.sha256);
...
try {
  ...
  assert.equal(await desktop.evaluate(()=>deadzone.snapshot().assetsLoaded.length),14);
  assert.equal(await desktop.locator('.weapon-preview').count(),4);
  ...
  const report={date:new Date().toISOString(),url,version,resources,bgm,og:{url:og,width:1734,height:907,bytes:imageBytes.length,sha256:crypto.createHash('sha256').update(imageBytes).digest('hex'),publicFetch:true},scope,models:14,previews:4,desktopCombat:true,sizes,errors,physicalDevicesTested:false,socialPostTested:false};
